      ...
      <div className="w-full grid lg:grid-cols-4 grid-cols-1 lg:gap-x-4 gap-y-4">
        {sections.map((item) => (
          <div
            ...
            key={item.id}
            className="border-t overflow-hidden cursor-pointer  border-[#E7E7E733] flex flex-col gap-y-5 border-b relative"
          >
            <span className="mt-3 text-[#D8D5FC] font-normal text-lg">
              {item.title}
            </span>
            <img
              src={item.img}
            ...
            <div className="flex flex-col items-start mb-5 relative">
              <span className="text-[15.25px] font-medium text-[#D8D5FC]">
                {item.subtitle}
              </span>
              <span className="font-normal text-sm text-[#E0DDFFA1]">
                {item.description}
              </span>
